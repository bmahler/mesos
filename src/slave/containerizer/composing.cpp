    ...
    const ContainerID& containerId)
{
  ...
  switch (container->state) {
    ...
    case LAUNCHING:
      ...
      // a containerizer is expected to handle a destroy while a
      // launch is in progress. If the containerizer could not handle
      // launching the container (`launch()` returns false), then the
      // containerizer may no longer know about this container. If
      // the launch returns false, we will stop trying to launch
      // the container on other containerizers.
      container->containerizer->destroy(containerId)
        .onAny(defer(self(), [=](const Future<bool>& destroy) {
          ...
          // return a successful destroy (in `_launch()`) when
          // the containerizer cannot handle this type of container
          // (`launch()` returns false).
          if (containers_.contains(containerId)) {
