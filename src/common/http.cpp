...
// change. Hopefully that doesn't prove too big of an issue..
  ...
  foreach (const FieldDescriptor* field, fields) {
    if (field->is_repeated() && !field->is_map()) {
      ...
      auto writeField = [&writer](
        ...
        switch (field->cpp_type()) {
          ...
          case FieldDescriptor::CPPTYPE_ENUM:
            writer->field(
                fieldName,
                slaveToAgent(reflection->GetEnum(message, field)->name()));
            break;
          case FieldDescriptor::CPPTYPE_STRING:
            const std::string& s =
      ...
      if (!field->is_repeated()) { // Singular field.
        writeField(slaveToAgent(field->name()), reflection, message, field);
      } else { // Map field.
        CHECK(field->is_map());
        writer->field(
            slaveToAgent(field->name()),
