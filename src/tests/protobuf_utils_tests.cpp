...
TEST(ProtobufUtilTest, AdjustAllocationInfoInOfferOperation)
{
  ...
  Offer::Operation adjustedLaunch =
    protobuf::adjustOfferOperation(launch, allocationInfo);

  ASSERT_EQ(1, adjustedLaunch.launch().task_infos_size());

  EXPECT_EQ(allocatedResources,
            adjustedLaunch.launch().task_infos(0).resources());

  EXPECT_EQ(allocatedResources,
            adjustedLaunch.launch().task_infos(0).executor().resources());
  ...
  Offer::Operation adjustedLaunchGroup =
    protobuf::adjustOfferOperation(launchGroup, allocationInfo);

  ASSERT_EQ(1, adjustedLaunchGroup.launch_group().task_group().tasks_size());

  EXPECT_EQ(allocatedResources,
            adjustedLaunchGroup.launch_group().task_group().tasks(0)
              .resources());

  EXPECT_EQ(allocatedResources,
            adjustedLaunchGroup.launch_group().task_group().tasks(0).executor()
              .resources());

  // Test the RESERVE case. This should be constructing a valid
  // reservation, but for now this just sets the resources
  // in order to verify the allocation info injection.
  Offer::Operation reserve = RESERVE(resources);
  Offer::Operation adjustedReserve =
    protobuf::adjustOfferOperation(reserve, allocationInfo);

  EXPECT_EQ(allocatedResources, adjustedReserve.reserve().resources());

  // Test the UNRESERVE case. This should be constructing a valid
  // reservation, but for now this just sets the resources
  // in order to verify the allocation info injection.
  Offer::Operation unreserve = UNRESERVE(resources);
  Offer::Operation adjustedUnreserve =
    protobuf::adjustOfferOperation(unreserve, allocationInfo);

  EXPECT_EQ(allocatedResources, adjustedUnreserve.unreserve().resources());

  // Test the CREATE case. This should be constructing a valid
  // volume, but for now this just sets the resources
  // in order to verify the allocation info injection.
  Offer::Operation create = CREATE(resources);
  Offer::Operation adjustedCreate =
    protobuf::adjustOfferOperation(create, allocationInfo);

  EXPECT_EQ(allocatedResources, adjustedCreate.create().volumes());

  // Test the DESTROY case. This should be constructing a valid
  // volume, but for now this just sets the resources
  // in order to verify the allocation info injection.
  Offer::Operation destroy = DESTROY(resources);
  Offer::Operation adjustedDestroy =
    protobuf::adjustOfferOperation(destroy, allocationInfo);

  EXPECT_EQ(allocatedResources, adjustedDestroy.destroy().volumes());
}
