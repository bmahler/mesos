    ...
    const Option<string>& readonlyAuthenticationRealm)
{
  ...
  // invoke `spawn()` and other calls below.
  ...
  Try<http::Server> create = http::Server::create(
      ...
      [&](const network::Socket& socket, const http::Request& request) {
        return process_manager->handle(socket, Request(request));
      },
      { .scheme = scheme, .backlog = LISTEN_BACKLOG, });
...
void finalize(bool finalize_wsa)
{
  ...
  server->stop(); // TODO(benh): Await for `shutdown` to complete?
  delete server;
  server = nullptr;
  ...
  if (!connected) {
    ...
    connect(socket.get(), to.address)
      .onReady([=]() {
        ...
        // In order to avoid a race condition where internal::send()
        // is called after SocketManager::link() but before the socket
        // is connected, we initialize the 'outgoing' queue in
        // SocketManager::link() and then check if the queue has
        // anything in it to send during this connection
        // completion. When a subsequent call to SocketManager::send()
        // occurs we'll now just add the encoder to the 'outgoing'
        // queue, and when we complete the connection here we'll start
        // sending, otherwise when we call SocketManager::next() the
        // 'outgoing' queue will get removed and any subsequent call
        // to SocketManager::send() will take care of setting it back
        // up and sending.
        Encoder* encoder = next(socket.get());

        if (encoder != nullptr) {
          internal::send(encoder, socket.get());
        }
      });
  }
}
...
{
  // Try to connect, then attempt downgrade.
  return socket.connect(address)
    .recover([=](const Future<Nothing>& future) -> Future<Nothing> {
      if (future.isAbandoned() || future.isDiscarded()) {
        ...
        return Failure("Abandoned or discarded connect");
      }

      VLOG(1) << "Failed to connect to '" << address << "': "
              << future.failure();
  ...
  synchronized (mutex) {
    ...
    if (persist || temp) {
      ...
      if (outgoing.count(socket.get()) > 0) {
        ...
        // Initialize the outgoing queue (in case it isn't).
        outgoing[socket.get()];
      }
      ...
      // TODO(benh): Capture the future returned from `connect` so
      // that we can discard it either when we swap the implementing
      // socket or we shut down SocketManager.
      connect(socket.get(), address)
        .onReady(lambda::bind(
            ...
            [socket](Message& message) {
              // Receive and ignore data from this socket. Note that
              // we don't expect to receive anything other than HTTP
              // '202 Accepted' responses which we just ignore.
              size_t size = 80 * 1024;
              char* data = new char[size];
  ...
  synchronized (mutex) {
    ...
    if (sockets.count(s) > 0) {
      ...
      if (!outgoing[s].empty()) {
        ...
        // Check if this socket is a temporary and if so clean it up.
        //
        // TODO(benh): Optimize this if it is deemed to be expensive
        // in practice. The reasoning here is that most sockets SHOULD
        // NOT be temporaries so this check won't often and when it
        // does happen there won't be many temporaries.
...
void SocketManager::close(int_fd s)
{
  ...
  //   close(s);
  //
  // Because, for example, there could be a race between trying to do
  // send a response with SocketManager::send() or a process might be
  // responding to another Request (e.g., trying to do a sendfile)
  // since these things may be happening asynchronously we can't close
  // the socket yet, because it might get reused before any of the
  // above things have finished, and then we'll end up sending data on
  // the wrong socket! Instead, we rely on the last reference of our
  // Socket object to close the socket. Note, however, that since
  // socket is no longer in 'sockets' any attempt to send with it will
  // just get ignored.
  //
    ...
    Request&& request)
{
  ...
  if (libprocess(request)) {
    ...
    return parse(request)
      .then([this, request](MessageEvent* event) -> Response {
  ...
  if (use(receiver)) {
    ...
    deliver(receiver, new HttpEvent(new Request(std::move(request)), promise));
