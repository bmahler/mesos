...
{
public:
  // XXX copying name in every time is wasteful, and
  // copying out the Event from the queue is wasteful too.
  // Ideally we could zero copy but that's quite tricky!
  struct Event
  {
    ...
    enum {
      ...
      // XXX Add an Initialized event for race prevention? A caller that wants
      // to watch the file for changes needs to read it once the watch is initialized
      // otherwise, they might miss the change they are looking for if it changed
      // right before the inotify_add_watch. My assumption right now is that there
      // isn't an initial event generated by inotify... (verify this with tests).
      // XXX Now that add is synchronous, the caller could read the file after
      // adding it to fix any missed events.
  ...
  // Returns an error in many situations: e.g. we don't have read access to the
  // provided path, the path has already been watched (and not implicitly or
  // explicitly removed), the path doesn't exist, etc.
  // XXX Document needing to read the file after this, since changes could be
  // missed if one reads then adds.
